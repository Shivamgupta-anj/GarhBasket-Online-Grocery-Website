...
const handleSearch = (e:React.FormEvent)=>{
  ...
  return (
    <div className='pt-4 w-[95%] md:w-[85%] mx-auto pb-20'>
        ...
        className='flex flex-col sm:flex-row items-center justofy-between gap-4 mb-8 text-center sm:text-left'
        >
            <button
            onClick={()=> router.push("/")}
            className='flex items-center justify-center gap-2 bg-green-100 hover:bg-green-200 text-gray-700 font-semibold px-4 py-2 rounded-full transition w-full sm:w-auto'
            >
                <ArrowLeft size={18}/><span>Back</span>
            </button>
           <h1 className='w-full text-2xl md:text-3xl font-extrabold    text-gray-700 flex items-center justify-end gap-2'>
            <Package size={28} className='text-gray-600'/>Manage Groceries
            </h1>
        ...
        className='flex items-center bg-white border border-gray-200 rounded-full px-5 py-3 shadow-smooth mb-10 hover:shadow-lg transition-all max-w-lg mx-auto w-full'
        >
            ...
            {fillterd?.map((g,i)=>(
                ...
                >
                    ...
                    <div className='relative w-full sm:w-44 aspect-square rounded-xl overflow-hidden border border-gray-200' >
                        ...
                        <Image src={g.image || '/placeholder.png'} alt={g.name} fill className='object-cover hover:scale-110 transition-transform duration-500'/>
                    </div>
                    ...
                    <div className='flex-1 flex flex-col justify-between w-full'>
                        ...
                        <div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 '>
                            ...
                            <button className='bg-gray-700 text-white px-4 rounded-lg text-sm font-semibold flex items-center judtify-center gap-2 hover:bg-green-700 transition-all' onClick={()=> setEditing(g)}>
                                <Pencil size={15}/> Edit
                            </button>
                    ...
                    >
                        ...
                        <div className='space-y-4'>
                            <input
                                ...
                                className="w-full border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none"
                            />

                            <select className='w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-gray-500 outline-none bg-white'
                        ...
                        <div className='flex justify-end gap-3 mt-6'>
                            <button className='px-4 py-2 rounded-lg bg-gray-600 text-white flex items-center gap-2 hover:bg-green-700 transition-all'
                            onClick={handleEdit}
                            disabled={loading}
                            >
                                {loading? <Loader size={14}/>:"Edit Grocery"}
                            </button>
                            <button className='px-4 py-2 rounded-lg bg-gray-600 text-white flex items-center gap-2 hover:bg-red-700 transition'
